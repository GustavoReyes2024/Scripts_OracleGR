...
AS SELECT id,first_name, last_name, email
FROM employees
...
AS SELECT country_name, region_id, population, internet_extension
FROM wf_countries
...
CREATE OR REPLACE VIEW view_euro_countries
AS SELECT airports AS "ID", country_name AS "Country",
area AS "Area"
FROM wf_countries
...
FROM employees
WHERE department_id = 50
WITH CHECK OPTION  view_dept50_check;

DROP VIEW IF EXISTS view_dept50; -- ¡Corregido aquí!
...
SELECT last_name, hire_date
FROM employees
ORDER BY hire_date ASC -- Ordena por fecha de contratación ascendente (los más antiguos)
LIMIT 5